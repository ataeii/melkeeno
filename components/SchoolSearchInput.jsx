      ...
      {options.length > 0 && (
        <div className='absolute z-20 mt-1 w-full bg-white border border-gray-200 rounded-lg shadow-lg max-h-56 overflow-y-auto'>
          {options.map((s) => (
            <div
              key={s.id}
              className='flex items-center gap-1 hover:bg-orange-50 border-b border-gray-50 last:border-0'
            >
              <button
                onClick={() => handleSelect(s)}
                className='flex-1 min-w-0 text-right px-3 py-2 text-sm'
              >
              ...
              <BookmarkToggle type='school' id={s.id} className='w-8 h-8 flex-shrink-0 ml-2' />
            </div>
          ))}
        </div>
      )}
